...
function AppShell({ initialData, serverMonth, serverYear }: AppShellProps) {
  ...
  const [bankAccountBalances, setBankAccountBalances] = useState<{ bank_account_id: number; balance: number }[]>(
    initialData?.bankAccountBalances ?? []
  );
  ...
  const loadCoreData = async (showLoading = false, light = false) => {
    ...
    try {
      ...
      setBankAccountBalances(data.bankAccountBalances ?? []);
      if (!light) {
          ...
          <main className="content fade-in">
            {errorMessage ? (
              ...
              <>
                ...
                {mountedTabs.has('sip') && <div style={{ display: currentView === 'sip' ? undefined : 'none' }}><SIPTracker currentMonth={currentMonth} currentYear={currentYear} onPortfolioUpdate={triggerRefresh} frozenSip={monthlySummary?.savings_sip} activeBudgetMonth={activeBudgetMonth} /></div>}
                {mountedTabs.has('stocks') && <div style={{ display: currentView === 'stocks' ? undefined : 'none' }}><StockTracker currentMonth={currentMonth} currentYear={currentYear} onPortfolioUpdate={triggerRefresh} onPricesRefreshed={() => setStockRefreshTick(t => t + 1)} frozenShares={monthlySummary?.savings_shares} activeBudgetMonth={activeBudgetMonth} /></div>}
